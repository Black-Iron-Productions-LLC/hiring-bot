...
import { HiringBotError, HiringBotErrorType, botReportError, safeReply, unknownDBError } from '../evaluate/reply-util';
import { prisma } from '../../db';
import { roleArray, roleEnglishArray } from '../../evaluatorRole';
import { Evaluator, Role } from '@prisma/client';
...
module.exports = {
	...
	async execute(interaction: ChatInputCommandInteraction) {
        if (interaction.options.getSubcommand() === 'modify_evaluator') {
            ...
            let evaluator = await prisma.evaluator.upsert({
                where: {
                    discordID: user.id,
                },
                create: {
            ...
            if (action === 'HIRING_MANAGER') {
                let result = await prisma.evaluator.update({
                    where: {
                        id: evaluator.id,
                    },
                    data: {
            ...
            } else if (action === 'APPLICATION_MANAGER') {
